...
export const FloatingConnectButton: React.FC<
  FloatingConnectButtonProps
> = () => {
  ...
  const connectWithAll = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>): void => {
      ...
      const connectButtons: HTMLButtonElement[] = Array.from(
      ...
      connectButtons.forEach((button: HTMLButtonElement) => {
        setTimeout(() => {
          ...
          setTimeout(() => {
            const sendButton: HTMLButtonElement | undefined = Array.from(
              document.querySelectorAll("button")
            ).find((btn): btn is HTMLButtonElement => {
              const text = btn.textContent?.toLowerCase() || "";
              return text.includes("Follow") && btn.offsetParent !== null;
            });
